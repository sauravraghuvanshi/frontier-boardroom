"""Prep-mode smoke test. Drives /api/v1/prep-session + /ws/prep over loopback."""
import asyncio
import json
import sys
...
async def collect_turn(ws, label: str) -> dict:
    """Drain WS until turn_end. Returns the assembled turn."""
    ...
    while True:
        ...
        if et == "user_message":
            ...
            print(f"  [citation] {uri.split('/')[-1]} {evt.get('confidence', 0):.2f}", flush=True)
        elif et == "token":
...
async def main():
    # 1. Create prep session as CFO on sea-expansion.
    sess = post_json(
        "/api/v1/prep-session",
    ...
    async with websockets.connect(f"{WS}/ws/prep/{sid}", ping_interval=None) as ws:
        # First frame should be prep_ready.
    ...
    if fails:
        ...
        for f in fails:
            print(f"  - {f}")
        sys.exit(1)
    print("PASS: coach + drill + simulate all streamed end-to-end")
